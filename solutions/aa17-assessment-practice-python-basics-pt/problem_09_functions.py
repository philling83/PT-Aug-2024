...
class PizzaProcessor():
  ...
  def pizza_prepper(self, *num_toppings):
    orders = {}

    for order_num, toppings in enumerate(num_toppings):
      if not isinstance(toppings, int) or toppings < 0 or toppings > 10:
        return self._error
      else:
        if order_num % 2 == 1:
          orders[str(order_num)] = { "crust": "stuffed", "toppings": toppings }
        else:
          orders[str(order_num)] = { "crust": "thin", "toppings": toppings }
...
# print(pizza_processor.pizza_production(employee_name="Brad", time_started="3:00", time_ended="3:30"))
